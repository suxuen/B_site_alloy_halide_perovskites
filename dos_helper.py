...
#    print(keys)
    ...
    for i, key in enumerate(keys):
        # print(i, key, colors[i % ncolors], alpha[i % ncolors])
        ...
        if red_key != 'Total':
            red_key = red_key.split()[0].split('(')[0] + '('+red_key.split()[1]+')' if len(red_key.split())==2 else red_key.split()[0].split('(')[0]
        ...
        # print(i, key, label, colors[i % ncolors], alpha[i % ncolors])
        ...
        centers_dic[str(i)] = {'energies': [yi for yi in y if yi >= energy_lim[0] and yi <= 0.0],
                               'densities': [x[iii] for iii,yi in enumerate(y) 
                                             if yi >= energy_lim[0] and yi <= 0.0],
                               'color': colors[i % ncolors],
                               'key': key}
        # plot band centers. x = densities, y = energies 
        # if i in show_band_centers:
        #     sign = 1
        #     if min(x) < 0:
        #         sign = -1
        #     bandcenter = np.average([yi for yi in y if yi >= energy_lim[0] and yi <= 0.0],
        #                             weights = [x[iii] for iii,yi in enumerate(y) 
        #                                         if yi >= energy_lim[0] and yi <= 0.0])
        #     print(bandcenter)
        #     plplot([0, density_lim[1]*(sign)], [bandcenter,bandcenter], color=colors[i % ncolors],
        #             # alpha = alpha[i % ncolors] if type(alpha) == list else alpha,
        #             ls = '--', lw = 1)
...
#                      color=colors[i % ncolors],linestyle="--",linewidth=1,)
    ...
    # plot band centers
    for pair in show_band_centers:
        print(pair)
        allens = []
        alldens = []
        for p in pair:
            # print('CHECK', p, centers_dic.keys())
            print(p, centers_dic[str(p)]['key'])
            allens += centers_dic[str(p)]['energies'] # within bounds of lower plot to 0.0 eV
            alldens += [np.abs(den) for den in centers_dic[str(p)]['densities']]
            color = centers_dic[str(p)]['color']
        bandcenter = np.average(allens, weights = alldens)
        print(bandcenter)
        plplot([density_lim[0], density_lim[1]], [bandcenter,bandcenter], color=color,
                ls = '--', lw = 1)
...
#            plplot(plxm(), [0, 0], "k--", linewidth=2)
    ...
    pdoslabel = 'DOS (a.u.)' if pdos_label else "Density of states (a.u.)"
...
#            ax.axhline(y=0, color="k", linestyle="--", linewidth=1)
        ...
        return ax, {'centers_dic': centers_dic}
    else:
        if flip_axes:
...
#            plot.axhline(y=0, color="k", linestyle="--", linewidth=1)
        ...
        plot.legend()
        return plot, {'centers_dic': centers_dic}
